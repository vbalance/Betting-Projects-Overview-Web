...
const Hero: React.FC<HeroProps> = ({ projects }) => {
  ...
  return (
    <section className="bg-gradient-to-br from-neutral-900 via-neutral-900 to-neutral-900 py-16 relative overflow-hidden min-h-screen">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-3 mb-8">
            {projects.map((project) => (
              <div
                key={project.id}
                className="group bg-white/5 backdrop-blur-sm p-3 rounded border border-neutral-600/30 hover:border-neutral-500/50 cursor-pointer hover:bg-white/8 h-20 flex items-center transition-all duration-300 hover:shadow-lg hover:shadow-black/20 hover:-translate-y-0.5"
                onClick={() => scrollToProject(project.id)}
              >
                <div className="flex items-center space-x-2 w-full">
                  <div className={`w-9 h-9 bg-gradient-to-br ${project.gradient} rounded flex items-center justify-center group-hover:scale-105 transition-all duration-300 flex-shrink-0 shadow-md`}>
                    <i className={`${project.icon} text-white text-sm`}></i>
                  </div>
                  <div className="min-w-0 flex-1">
                    <h3 className="text-white text-sm font-semibold truncate group-hover:text-neutral-100 transition-colors">{project.title}</h3>
